...
export function initializeMachineControl({ getNcText, getBlockSetup, onPositionChange, onJobStateChange }) {
  ...
  const buildInstallationCard = () => {
    ...
    motionSummary.textContent = dynamics.findings.length || dynamics.advisories.length
      ? `Проаналізовано ${dynamics.segments.length} рухів: небезпек ${dynamics.dangerCount}, попереджень ${dynamics.warningCount}, службових ${dynamics.advisories.length}, зон ризику пропалу ${dynamics.burnThroughRiskCount}; найбільша F${dynamics.maximumProgramFeed}.`
